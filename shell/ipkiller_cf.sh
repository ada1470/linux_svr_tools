...
run_firewall_cmd() {
    ...
    if [[ "$FIREWALL_CMD" == "firewalld" ]]; then
        case "$action" in
            add) run_firewall_cmd add "$rule" ;;
            remove) run_firewall_cmd remove "$rule" ;;
            reload) run_firewall_cmd reload ;;
            list) run_firewall_cmd list ;;
            backup) run_firewall_cmd backup ;;
        esac
