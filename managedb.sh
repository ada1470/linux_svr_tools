...
while true; do
    ...
    echo "9) Login to mysql"
    echo "10) Restart mysql"
    echo "0) Exit"
    ...
    case $choice in
        ...
        8)
            ...
            databases=($(mysql -uroot -p"$mysql_pwd" -e "SHOW DATABASES;" \
                        | tail -n +2 \
            ...
            # Ask user to choose a database by number
            read -p "Enter the number of the database to show its tables: " choice
            ...
            # Show tables for the selected database
            echo -e "\n📂 Tables in $selected_db:"
            mysql -uroot -p"$mysql_pwd" -e "USE \`$selected_db\`; SHOW TABLES;" \
                  | tail -n +2 | sed 's/^/ - /'

            ;;
        9)
            echo -e "\n📋 Available databases:"
            mysql -uroot -p"$mysql_pwd" -e "SHOW DATABASES;" | tail -n +2 | grep -Ev "^(information_schema|mysql|performance_schema|sys)$"
...
EOF
            echo -e "\n---------------------------------------"
            # Start interactive MySQL session
